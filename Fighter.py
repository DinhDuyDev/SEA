...
class Fighter:
    ...
    def shotgun_enemy(self):
        ...
        if self.ROF > rof_end:
            for i in range(7):
                ...
                create_bullet(self.x, self.y, direction_to_target, self, damage=3)
            self.ROF = 0
        self.ROF += 1
    ...
    def machine_gun_enemy(self):
        ...
        self.weapon_direction = direction_to_target
        if self.ROF > rof_end:
            create_bullet(self.x, self.y, direction_to_target, self, 2)
            self.ROF = 0
        self.ROF += 1
    ...
    def splitter_enemy(self):
        ...
        self.weapon_direction = direction_to_target
        if self.ROF > rof_end:
            create_bullet(self.x, self.y, direction_to_target, self, 1.75, bullet_type="Splitter")
            self.ROF = 0
        self.ROF += 1
...
def add_fighter(x=random.randrange(0, 640), y=random.randrange(0, 320)):
    Fighter.list_of_fighters.append(Fighter(x, y, Fighter.full_health))
